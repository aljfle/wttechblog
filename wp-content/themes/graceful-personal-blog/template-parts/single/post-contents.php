...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="post-page-content">
		...
		$graceful_defaults = array(
			'before' => '<p class="site-pagination">' . esc_html__( 'Pages:', 'graceful-personal-blog' ),
			'after' => '</p>'
		);
	...
	<footer class="post-footer">
		...
		if ( graceful_options( 'single_post_page_show_comments' ) && comments_open() ) {
			comments_popup_link( esc_html__( 'No Comments', 'graceful-personal-blog' ), esc_html__( '1 Comment', 'graceful-personal-blog' ), '% ' . esc_html__( 'Comments', 'graceful-personal-blog' ), 'post-comments');
		}
		?>
	</footer>
